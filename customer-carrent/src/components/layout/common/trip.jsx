...
function InforCustomer() {
	...
	const getCustomerTrip = async () => {
		try {
			const response = await axiosConfig.get(`http://localhost:8080/api/v1/contracts/customer-trip`);
	...
	const handleCancel = async (contractId) => {
		try {
			const response = await fetch(`http://localhost:8080/api/v1/contracts/delete/${contractId}`, {
				method: "DELETE",
				headers: {
									...
									<div>
										{contract.remainBill == 0 ? (
											...
											<button
												...
												onClick={() =>
													handlePayRemain(contract.contractId, contract.remainCost)
												}
											>
												{t("remainPayment")}
											</button>
								...
								<div>
									...
									<button
										...
										onClick={() => {
											...
											navigate(`/carrentsg/car/${contract.carRegistrationPlate}`);
										}}
									>
						...
						<div className="content">
							<div className="avatar-box">
								<div className="avatar-trip avatar--xl">
									<img loading="lazy" src={contract.carImage} alt={contract.carName} />
								</div>
								<h6>{contract.carName}</h6>
								<p className="note">
									{t("start_date")}: {formatDate(contract.createDate)}
								</p>
							</div>
							<div className="info-user">
								<div className="info-box">
									...
									<div className="info-box__item">
										<p>{t("total_cost")}</p>
										<p className="main">{formatVND(contract.totalRentCost)} VND</p>
									</div>
									<div className="info-box__item">
										<p>{t("deposit")}</p>
										<p className="main">{formatVND(contract.totalRentCost * 0.2)} VND</p>
									</div>
									<div className="info-box__item">
										<p>{t("pay_later")}</p>
										<p className="main">
											{formatVND(
												contract.remainBill == 0 ? contract.remainCost : contract.remainBill
											)}{" "}
											VND
										</p>
									</div>
